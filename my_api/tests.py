...
class MyApiTests(TestCase):
    ...
    def test_author_detail_endpoint(self):
        response = self.client.get(reverse("authors_details", args=[self.author1.id]))
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_author_update_endpoint(self):
        ...
        response = self.client.put(
            reverse("authors_details", args=[self.author1.id]),
    ...
    def test_author_deletion_endpoint(self):
        response = self.client.delete(reverse("authors_details", args=[self.author1.id]))
    ...
    def test_get_book_detail(self):
        response = self.client.get(reverse("books_details", args=[self.book1.id]))
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_create_book(self):
        ...
        response = self.client.post(
            reverse("books"), json.dumps(data), content_type="application/json"
        )
        self.assertEqual(response.status_code, 201)
        data = response.json()
        self.assertIn("id", data)
    ...
    def test_update_book(self):
        ...
        response = self.client.put(
            reverse("books_details", args=[self.book1.id]),
            json.dumps(data),
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_book(self):
        response = self.client.delete(reverse("books_details", args=[self.book1.id]))
        self.assertEqual(response.status_code, 204)
